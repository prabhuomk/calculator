...
var minusChecker=false;   //to differentiate - as operator and sign in display ,used in function digit();
var operatorChecker=false;  //to clear color of other operator when new operator is pressed
var dotCounter=0; // to prevent multiple dots,used in function digit() in dot part;
var openCount=0;  // count number of open bracket
var closeCount=0; // count number of close bracket 
var insideSquare="";
var swapCal=0;
...
//function when digits is typed

function digits(value) 
{
   
        // all digits except for dot
  
  if(value!='.')
    {
         if(minusChecker==true || document.getElementById("display").value=="0")// to not display minus operator but display minus when it is sign
         {
...
// for operator two function is called first function task() then  function opearator()
// function task - it prevent various illegal use of operator
// function operator - store the operator
...
function task(value)
{
	...
	if(value=="-" && signChecker!='.')
	{
	   ...
	   if(store.length==0||signChecker=='+' ||signChecker=='-'||signChecker=='*'||signChecker=='/')
	   {
	                       ...
	                       //condtion to prevent -- at start & --- other then start
	                       
	        if(store.charAt(0)!='-' && signChecker2!='+'&& signChecker2!='-'&& signChecker2!='*'&& signChecker2!='/')
	        {
	...
	else if(store!="-" && store.length>=1 && value!='(' && value!=')')
	{
	                       ...
	                       //select last opertaor /+*
	        if(signChecker=='+'||signChecker=='*'||signChecker=='/'||(signChecker=='-'&& signChecker2!='+'&& signChecker2!='-'&& signChecker2!='*'&& signChecker2!='/'))
	        {      
...
// function for all special operator like sin,cos etc
...
// function to check order of precedence
...
// function to perform different operation
